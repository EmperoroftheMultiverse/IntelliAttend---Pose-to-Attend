...
export const initializeNewInstitute = functions.https.onCall(async (data, context) => {
  ...
  try {
    const instituteRef = await db.collection("institutes").add({ name: instituteName });
    const instituteId = instituteRef.id;

    // Set BOTH instituteId and role as custom claims
    ...
    await db.collection("institutes").doc(instituteId).collection("users").doc(user.uid).set({
      name: user.displayName || 'Admin', email: user.email, role: "admin",
    });
...
export const createUser = functions.https.onCall(async (data, context) => {
  // Check for role directly on the auth token
  ...
  const { email, password, name, role, faceDescriptor, year } = data;
